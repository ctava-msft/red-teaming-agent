...
module api './app/api.bicep' = if (!vnetEnabled) {
  name: 'api'
  scope: rg
  params: {
    ...
    storageAccountPrimaryEndpoints: storage.outputs.primaryEndpoints
...
module apiWithVnet './app/api.bicep' = if (vnetEnabled) {
  name: 'api-with-vnet'
  scope: rg
  params: {
    name: functionAppName
    ...
    virtualNetworkSubnetId: '${rg.id}/providers/Microsoft.Network/virtualNetworks/${serviceVirtualNetworkName}/subnets/${serviceVirtualNetworkAppSubnetName}'
  }
}
